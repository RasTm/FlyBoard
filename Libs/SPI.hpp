...
class SPI_Base{
	...
	SPI_Base(SPI_TypeDef *SPIxn, uint8_t Mode, uint8_t NSS_Mode, uint8_t baud_rate_div, bool DFF, bool LSBFirst){
		...
		if(SPIxn == SPI1){
			...
			gpio_config(GPIOA, 5, ALTER, PUSH, HIGH, NOT, SPI1_2);             //SCK Pin

			if(NSS_Mode != NSS_DISABLE){
				gpio_config(GPIOA, 4, ALTER, PUSH, HIGH, UP, SPI1_2);          //NSS Pin
			}
			if(Mode == FULL_DUPLEX_MASTER || Mode == FULL_DUPLEX_SLAVE){
				gpio_config(GPIOA, 6, ALTER, PUSH, HIGH, UP, SPI1_2);          //MISO Pin
				gpio_config(GPIOA, 7, ALTER, PUSH, HIGH, NOT, SPI1_2);         //MOSI Pin
			}
			...
			gpio_config(GPIOB, 10, ALTER, PUSH, HIGH, NOT, SPI1_2);            //SCK Pin

			if(NSS_Mode != NSS_DISABLE){
				gpio_config(GPIOB, 12, ALTER, PUSH, HIGH, UP, SPI1_2);         //NSS Pin
			}
			if(Mode == FULL_DUPLEX_MASTER || Mode == FULL_DUPLEX_SLAVE){
				gpio_config(GPIOC, 2, ALTER, PUSH, HIGH, UP, SPI1_2);          //MISO Pin
				gpio_config(GPIOC, 3, ALTER, PUSH, HIGH, NOT, SPI1_2);         //MOSI Pin
			}
			else if(Mode == HALF_DUPLEX_MASTER || Mode == RECEIVE_ONLY_SLAVE  || Mode == TRANSMIT_ONLY_MASTER){
				gpio_config(GPIOC, 3, ALTER, PUSH, HIGH, NOT, SPI1_2);         //MOSI Pin
				SPIx-> CR1 |= 0x8000;                                          //SPI BIDIMODE Enable
			...
			gpio_config(GPIOC, 10, ALTER, PUSH, HIGH, NOT, SPI_3);              //SCK Pin

			if(NSS_Mode != NSS_DISABLE){
				gpio_config(GPIOA, 15, ALTER, PUSH, HIGH, UP, SPI_3);           //NSS Pin
			}
			if(Mode == FULL_DUPLEX_MASTER || Mode == FULL_DUPLEX_SLAVE){
				gpio_config(GPIOC, 11, ALTER, PUSH, HIGH, UP, SPI_3);           //MISO Pin
				gpio_config(GPIOC, 12, ALTER, PUSH, HIGH, NOT, SPI_3);          //MOSI Pin
			}
			else if(Mode == HALF_DUPLEX_MASTER || Mode == RECEIVE_ONLY_SLAVE  || Mode == TRANSMIT_ONLY_MASTER){
				gpio_config(GPIOC, 12, ALTER, PUSH, HIGH, NOT, SPI_3);          //MOSI Pin
				SPIx-> CR1 |= 0x8000;                                           //SPI BIDIMODE Enable
