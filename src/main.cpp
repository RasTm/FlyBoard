...
//	Serial.USART_Transmit(clear_line);
	...
	while (1){
		...
		if(program_buffer[read_index] == MPU6050_FLAG){
			mpu_Hz_counter++;
		...
		if(program_buffer[read_index] == MS5611_FLAG){
			ms_Hz_counter++;
...
extern "C" { void TIM8_TRG_COM_TIM14_IRQHandler(void){
		...
		if(buffer_write_able == true){
			if(mpu_tick > 2){
				mpu_tick = 0;
				if(write_index < 12){
					program_buffer[write_index] = MPU6050_FLAG;
					write_index++;
				}
			}
			if(ms_tick > 11){
				ms_tick = 0;
				if(write_index < 12){
					program_buffer[write_index] = MS5611_FLAG;
					write_index++;
				}
			}
			if(hmc_tick > 10){
				hmc_tick = 0;
				if(write_index < 12){
					program_buffer[write_index] = HMC5883_FLAG;
					write_index++;
				}
			}
		}
